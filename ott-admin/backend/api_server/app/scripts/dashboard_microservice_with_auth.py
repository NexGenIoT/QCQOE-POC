...
logging.basicConfig(filename="api_server_logs5.log", level=logging.DEBUG,
...
def get_data():
    ...
    #key["to_tome"]
    key="metrics"+"_"+json.dumps({**filters,**{"from_time":req.args.get('from_time'),"to_time":req.args.get('to_time')}})    
    data=redis.get(key)
    if data is not None:
        ...
        if len(filters["device_platform"]) != 0:
            ...
            for i in filters["device_platform"]:
                ...
                output_response.append(
                    {i: {"time_stamp": time_stamp, filters["metricname"]: metric_value}})
            if len(output_response)>0:
                redis.set("metrics"+"_"+json.dumps(collections.OrderedDict({**filters,**{"from_time":req.args.get('from_time'),"to_time":req.args.get('to_time')}})),json.dumps(output_response))            
            new_df=pd.DataFrame()
            ret={}
            ...
            for i in filters["device_platform"]:
                ret={}
                ret[i]={}
                ret[i][filters["metricname"]]=list(new_df[filters["metricname"]+"_"+i].values)
            ...
            if len(response["aggregations"]["selected_dates"]["time_stamp"]["buckets"]) > 0:
                for j in response["aggregations"]["selected_dates"]["time_stamp"]["buckets"]:
                    ...
                    if j["average_value"]["value"] is None:
                        ...
                        metric_value.append(j["average_value"]["value"])
            output_response.append(
                {"all": {"time_stamp": time_stamp, filters["metricname"]: metric_value}})
            if len(output_response)>0:
                redis.set("metrics"+"_"+json.dumps(collections.OrderedDict({**filters,**{"from_time":req.args.get('from_time'),"to_time":req.args.get('to_time')}})),json.dumps(output_response),ex=18000)
            return jsonify(output_response)
...
def get_data_dev():
    ...
    if req.args.get('from_time') is not None and len(str(req.args.get('from_time'))) != 0:
        ...
        time_end = ""
    filters = req.json
    data=redis.get("aggregated_data_for_24hrs"+"_"+json.dumps(collections.OrderedDict({**filters,**{"from_time":req.args.get('from_time'),"to_time":req.args.get('to_time')}})))
    app.logger.info("gullu"+str("aggregated_data_for_24hrs"+"_"+json.dumps(collections.OrderedDict({**filters,**{"from_time":req.args.get('from_time'),"to_time":req.args.get('to_time')}}))))
    if data is not None:
        ...
        if len(response["hits"]["hits"]) == 0:
            ...
            if filters["group_on"] == "content_partner":
                ...
                sso_percentage = np.random.randint(40, 80, main_df.shape[0])
                sso_errors = np.random.randint(1000, 1500, main_df.shape[0])
                main_df["sso %"] = sso_percentage
                main_df["sso Errors"] = sso_errors
                try:
                    main_df.drop(main_df[main_df["content_partner"]== ""].index[0], inplace=True)
                except:
                    app.logger.info("No Data For  Key")
                try:
                    main_df.drop(main_df[main_df["content_partner"]=="amzn_prm_logo"].index[0], inplace=True)
                except:
                    app.logger.info("No Data For  Key")
                try:
                    ...
                    app.logger.info("No Data For  Key")
                # --------------- Removing data and dropping unwanted partner's data
                #main_df.drop(main_df[main_df["content_partner"]== "Sun NXT"].index[0], inplace=True)
                for i in ["HotStar", "Amazon Prime Video", " Disney+ Hotstar", "Zee5", "SonyLIV"]:
                    try:
            ...
            if filters["group_on"] == "content_partner":
                ...
                if len(json_out)>0:
                    app.logger.info("gullu"+str("aggregated_data_for_24hrs"+"_"+json.dumps(collections.OrderedDict({**filters,**{"from_time":req.args.get('from_time'),"to_time":req.args.get('to_time')}}))))
                    redis.set("aggregated_data_for_24hrs"+"_"+json.dumps(collections.OrderedDict({**filters,**{"from_time":req.args.get('from_time'),"to_time":req.args.get('to_time')}})),json.dumps(json_out),ex=300)
                return jsonify(json_out)
            else:
                if len(json_out)>0:
                    redis.set("aggregated_data_for_24hrs"+"_"+json.dumps(collections.OrderedDict({**filters,**{"from_time":req.args.get('from_time'),"to_time":req.args.get('to_time')}})),json.dumps(main_df.to_dict(orient='records')),ex=300)
...
def percentage_change():
    ...
    if req.args.get('from_time_24hrs') is not None and len(str(req.args.get('from_time_24hrs'))) != 0:
        ...
        time_start_48hrs = ""
    filters = req.json
    app.logger.info("filtters "+str(filters))
    data=redis.get("percentage_change"+"_"+json.dumps(collections.OrderedDict({**filters,**{"from_time_24hrs":req.args.get('from_time_24hrs'),"from_time_48hrs":req.args.get('from_time_24hrs'),"to_time":req.args.get('to_time')}})))
    start=time.time()
    app.logger.info(str("percentage_change"+"_"+json.dumps(collections.OrderedDict({**filters,**{"from_time_24hrs":req.args.get('from_time_24hrs'),"from_time_48hrs":req.args.get('from_time_24hrs'),"to_time":req.args.get('to_time')}}))))
    app.logger.info("tame:"+str(time.time()-start))
    if data is not None:
        app.logger.info(data)
        #data=json.loads(data)
        return data
        ...
        for metricname in aggregation_type.keys():
            query_24hrs = generate_query_for_groupby_percentage_change(
                time_start_24hrs, time_end, filters, aggregation_type[metricname], metricname, filters["interval"]["current"])
            ...
            query_48hrs = generate_query_for_groupby_percentage_change(
                time_start_48hrs, time_end, filters, aggregation_type[metricname], metricname,filters["interval"]["last"])
        ...
        for i in percentage_change.keys():
            if i in ["video_start_failures","video_playback_failures","user_attrition","rebuffering_percentage","rebuffering_ratio","exit_before_video_starts"]:
                output_response.append({"percentage":percentage_change[i],"hrs_change_24hrs":results_24hrs[i],"metricname":i.replace("_"," ").title(),"hrs_change_48hrs":results_48hrs[i],"unit":units[i],"color":True if results_48hrs[i]>results_24hrs[i] else False})
            else:
                output_response.append({"percentage":percentage_change[i],"hrs_change_24hrs":results_24hrs[i],"metricname":i.replace("_"," ").title(),"hrs_change_48hrs":results_48hrs[i],"unit":units[i],"color":True if results_48hrs[i]<results_24hrs[i] else False})                
        app.logger.info(req.json)
        app.logger.info(str("percentage_change"+"_"+json.dumps(collections.OrderedDict({**filters,**{"from_time_24hrs":req.args.get('from_time_24hrs'),"from_time_48hrs":req.args.get('from_time_24hrs'),"to_time":req.args.get('to_time')}}))))
        redis.set("percentage_change"+"_"+json.dumps(collections.OrderedDict({**req.json,**{"from_time_24hrs":req.args.get('from_time_24hrs'),"from_time_48hrs":req.args.get('from_time_24hrs'),"to_time":req.args.get('to_time')}})),json.dumps(output_response),ex=300)
        return jsonify(output_response)
