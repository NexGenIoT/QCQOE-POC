...
export default function FilterDetectedAnamoliesLayout(props) {
    ...
    useEffect(() => {
      setCheckedItemsDeviceId(deviceId);
      saveDeviceIdData(deviceId);
    }, [deviceId]);
    ...
    useEffect(() => {
        setCheckedItemLiveData(live);
        saveLiveData(live);
    }, [live]);

    useEffect(() => {
        setCheckedItemDrmData(drm);
        saveDrmData(drm);
    }, [drm]);

    useEffect(() => {
        setCheckedItemHasData(has);
        saveHasData(has);
    }, [has]);
